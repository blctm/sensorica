...
def extract_excel_to_dataframe_simple(uploaded_file):
    """
    Versión simplificada para casos donde la estructura es más estándar
    """
    try:
        ...
        for i in range(min(5, len(df))):
            ...
            if any('def' in val or 'temp' in val or 'hum' in val for val in row):
                # Usar esta fila como encabezado
                df.columns = df.iloc[i]
                df = df[i+1:].reset_index(drop=True)
                break
        else:
            # Si no se encuentra, usar la primera fila
